...
def make_country_weights_ls_vol(
    ...
    vol_lookback=36,        # 3-year rolling window for vol targeting
):
    """
    CROSS-SECTIONAL COUNTRY STRATEGY WITH VOLATILITY TARGETING
    ----------------------------------------------------------
    - AQR-style long/short construction based on cross-sectional z-scores.
    - A country is eligible at time t only if its next-month return is available.
    - Ranking and z-scoring are performed only across eligible countries.
    - weights_raw contains the unscaled z-score weights.
    - Volatility targeting is applied at the portfolio level using a 36-month
      rolling volatility estimate based on strategy returns.
    - For the first 36 months, no volatility forecast is available, so
      weights_scaled will be NaN for those initial periods (expected behavior).
    """
    ...
    # -----------------------------
    # 1. Align data & apply optional lag
    # -----------------------------
    if signal_lag > 0:
        signal = signal.shift(signal_lag)
    ...
    # -----------------------------
    # A country is usable at time t only if its next-month return exists
    has_next = returns.shift(-1).notna()

    # A date is valid only if at least `min_regions` countries are usable
    valid_dates = has_next.sum(axis=1) >= min_regions

    # -----------------------------
    # 3. Construct unscaled cross-sectional z-score weights
    # -----------------------------
    out = []

    for t in ranks.index[valid_dates]:

        # Countries eligible at time t
        good = has_next.loc[t]
        usable_cols = good[good].index

        # Restrict ranks to eligible countries
        r_t = ranks.loc[t, usable_cols].dropna()
        if len(r_t) < min_regions:
            continue
    ...
    # Strategy returns using lagged weights
    strat_ret_raw = (weights_raw.shift(1) * returns_aligned).sum(axis=1)

    # Rolling annualized volatility estimate
    ...
    # Scaled weights (NaN for the first 36 months – expected)
    weights_scaled = weights_raw.mul(scaling, axis=0)
    weights_scaled = weights_scaled.dropna(how='all')
